...
function Product() {
  ...
  const onSubmit = async (data) => {
    ...
    if (data.productimage && data.productimage[0]) formdata.append("productimage", data.productimage[0])
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-10 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-4xl overflow-hidden rounded-[32px] bg-white shadow-2xl ring-1 ring-slate-200">
        <div className="bg-gradient-to-r from-sky-600 via-cyan-500 to-emerald-500 px-8 py-10 text-white sm:px-12 sm:py-12">
          <p className="text-sm font-semibold uppercase tracking-[0.24em] text-cyan-100">Product Form</p>
          <h1 className="mt-3 text-3xl font-semibold sm:text-4xl">Add a New Product</h1>
        ...
        <div className="px-6 py-8 sm:px-10 sm:py-10">
          <form className="space-y-6" onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data">
            ...
            <div>
              ...
              <input {...register("productimage", { required: true })} id="productImage" type="file" accept="image/*" className="w-full rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-700 file:mr-4 file:rounded-full file:border-0 file:bg-slate-200 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-slate-700 focus:outline-none" />
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-slate-500">Add the product details and save to update the store inventory.</p>
              <button type="submit" className="inline-flex items-center justify-center rounded-3xl bg-sky-600 px-6 py-3 text-sm font-semibold text-white transition hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-200">Save Product</button>
            </div>
